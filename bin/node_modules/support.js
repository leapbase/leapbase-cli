...
exports.createProject = async function(projectName, downloadUrl) {
  ...
  if (!downloadUrl) {
    downloadUrl = await exports.getLastestRelease(githubUsername, githubProjectName).catch(error => {
      console.log(`Error in getting latest release url for ${githubUsername}/${githubProjectName}`);
      return false;
    });
  }
  if (!downloadUrl) { 
  ...
  await exports.downloadFile(downloadUrl, downloadFile).catch(error => {
    console.log(`Error in downloading ${downloadUrl}`);
    process.exit();
  });
...
exports.addExtension = async function(extensionName, downloadUrl) {
  ...
  if (!downloadUrl) {
    downloadUrl = await exports.getLastestRelease(githubUsername, githubProjectName).catch(error => {
      console.log(`Error in getting latest release url for extension ${extensionName}`);
      process.exit();
    });
  }
  console.log('release file:', downloadUrl);
